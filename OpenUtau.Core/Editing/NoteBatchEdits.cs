    ...
    public class AutoTunePitch : BatchEdit {
        ...
        public async void Run(UProject project, UVoicePart part, List<UNote> selectedNotes, DocManager docManager) {
            ...
            if (File.Exists(path)) {
                ...
                model = Classic.Data.Resources.pitch;
            }
            using InferenceSession session = Onnx.getInferenceSession(model);
            ...
            foreach (var phrase in phrases) {
                ...
                var output = await Task.Run(() => {
                    ...
                    using var outputTensor = session.Run(inputs);
                    var output = outputTensor.First().AsTensor<float>().ToArray();
